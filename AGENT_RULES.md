...
export function useUpdateProp() {
  ...
  return useMutation({
    ...
    mutationFn: ({ id, payload }: { id: string; payload: UpdatePropPayload }) => {
...
function validatePropForm(legalName: string, address: AddressFormValue): string | null {
...
import type { Feature, CreateFeaturePayload, UpdateFeaturePayload } from '@/domain/[feature]'

class FeatureApi extends BaseService<Feature, CreateFeaturePayload, UpdateFeaturePayload> {
  constructor() {
    super('[feature]')
  }
}
